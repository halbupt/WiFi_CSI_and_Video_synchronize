...
%     x(cellfun(@isempty,x))=[]; %remove the empety elements,
%     y(cellfun(@isempty,y))=[]; %remove the empety elements,
%     x_l(cellfun(@isempty,x_l))=[]; %remove the empety elements,
%     y_l(cellfun(@isempty,y_l))=[]; %remove the empety elements,
%     train_data = cat(1,x{:});% [num*270*200]
%     train_label = cat(1, y{:});
%     test_data = cat(1,x_l{:});
%     test_label = cat(1, y_l{:});
